<template>
    <div class="brand_collection">
        <div v-if="products.length > 0">
            <h5>GIỚI THIỆU BỘ SƯU TẬP <span style="font-weight: bold;">{{ brands.find(b => b._id ==
                this.$route.params.id)?.name }}</span></h5>
            <p class="ml-4 font-italic">{{ brandCollection.description }}</p>
        </div>
        ...
        <div v-else class="font-weight-bold mt-4">
            <h5 class="text-center font-weight-bold ">
                Hiện tại không có sản phẩm nào trong bộ sưu tập thuộc nhãn hiệu "<span class="text-danger">{{
                    brands.find(b =>
                        b._id ==
                        this.$route.params.id)?.name }}</span>"</h5>
        </div>
...
export default {
    ...
    methods: {
        ...
        async fetchBrandCollection() {
            try {
                this.brandCollections = await collectionService.getAll();
                this.brandCollection = this.brandCollections.find(b => b.brandId == this.$route.params.id); // id là brand id
                console.log("hâhaa", this.brandCollection);
            } catch (error) {
                console.log(error);
            }
        },
        ...
        async fetchProducts() {
            try {
                ...
                this.products = this.products.filter(pro => pro.brandId == this.$route.params.id);
    ...
    async mounted() {
        ...
        socket.on('createProduct', async (data) => {
            console.log(data);
            await this.fetchBrandCollection();
