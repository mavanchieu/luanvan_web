<template>
    <div class="seasonal-coll">
        <div v-if="filters.length > 0">
            <h5>GIỚI THIỆU <span style="font-weight: bold;">{{ seasonalCollection.collectionName }}</span></h5>
            <p class="ml-4 font-italic">{{ seasonalCollection.description }}</p>
        </div>
        <div class="product" v-if="filters.length > 0">
        ...
        <div v-else class="font-weight-bold mt-4">
            <h5 class="text-center font-weight-bold ">
                Hiện tại không có sản phẩm nào trong bộ sưu tập!</h5>
        </div>
        <div class="pagination" v-if="filters.length > 0">
    ...
    methods: {
        ...
        async fetchSeasonalCollections() {
            try {
                ...
                this.seasonalCollection = this.seasonalCollections.find(s => s._id == this.$route.params.id);
        ...
        async fetchFilters() {
            this.filters = this.products.filter(p => this.seasonalCollection.productIds.some(s => s == p._id));
            this.totalPages = Math.ceil(this.filters.length / this.itemsPerPage);
        },
